...
// Send to your verified email first
const verifiedEmail = 'shubhamkush012@gmail.com';
...
<div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; background-color: #1e293b; color: #ffffff; padding: 20px; border-radius: 10px;">
  ...
  <div style="text-align: center; margin-bottom: 30px;">
    <h2 style="color: #f97316; margin: 0; font-size: 20px;">Hi Shubham! 👋</h2>
...
async function sendTestEmail() {
  console.log('🧪 Testing email delivery to verified address...');
  ...
  try {
    const result = await resend.emails.send({
      ...
      to: [verifiedEmail],
      subject: '🧪 Test: Match Credentials - EpicEsports',
      html: emailTemplate,
      text: textTemplate,
    });
    ...
    if (result && result.data && result.data.id) {
      ...
      console.log(`📧 Check your inbox at: ${verifiedEmail}`);
    } else if (result.error) {
      console.error(`❌ Test email failed:`, result.error);
    } else {
      console.log(`⚠️  Test email sent but no message ID returned`);
